...
class fourier_model(nn.Module):
    ...
    def __init__(self, cfg, layer= fourier_layer):
        ...
        self.layer_blocks.append(layer(self.n_i, self.n_o, neuron_size= self.neuron_size, learn_type=self.learn_type, device= self.device, weights= self.weights[f'layer{self.n_layers-1}'], circular = self.circular))
